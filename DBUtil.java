...
public class DBUtil {
    ...
    public static int insert(String tableName, Map<String, Object> valueMap) throws SQLException {
        ...
        try {
            ...
            String[] keys = columnSql.toString().split(",");
            /**设置sql占位符中的值**/
            for (int j = 1; j < i; j++) {
                preparedStatement.setObject(j, valueMap.get(keys[j - 1]));
            }
            /**执行sql**/
            affectRowCount = preparedStatement.executeUpdate();
            connection.commit();
            System.out.println("成功插入了了" + affectRowCount + "行");
        } catch (Exception e) {
            if (connection != null) {
                connection.rollback();
            }
    ...
    public static int update(String tableName, Map<String, Object> valueMap, Map<String, Object> whereMap) throws SQLException {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                columnSql.append(iterator.next() + " = ? ");
                i++;
            }
            sql.append(columnSql);
            ...
            /**替换要更新的值的占位符的相应的值**/
            String[] valuesKeys = columnSql.toString().replace("= ?", "").split(",");
            System.out.println("valuesKeys=" + Arrays.toString(valuesKeys));
            /**设置sql占位符中的值**/
            for (int k = 0; k < i; k++) {
                System.out.println(k + 1 + "=" + valueMap.get(valuesKeys[k].trim()));
                preparedStatement.setObject(k + 1, valueMap.get(valuesKeys[k].trim()));
            }

            /**替换要更新的条件的占位符的相应的值**/
            String[] whereKeys = whereSql.toString().replace(" WHERE ", "").replace("= ?", "").split(" AND");
            System.out.println("whereKeys=" + Arrays.toString(whereKeys));
            /**设置sql占位符中的值**/
            for (int m = 0; m < j; m++) {
                System.out.println(m + j + 1 + "=" + whereMap.get(whereKeys[m].trim()));
                preparedStatement.setObject(m + j + 1, whereMap.get(whereKeys[m].trim()));
            }
            /**执行sql**/
            affectRowCount = preparedStatement.executeUpdate();
            /**想数据库提交**/
            connection.commit();
            System.out.println("成功更新了" + affectRowCount + "行");
        } catch (Exception e) {
            if (connection != null) {
                connection.rollback();
            }
...
//            /**设置sql占位符中的值**/
//            for (int j = 0; j < i; j++) {
//                System.out.println(j + 1 + "=" + whereMap.get(whereKeys[j].trim()));
//                preparedStatement.setObject(j + 1, whereMap.get(whereKeys[j].trim()));
//            }
//            /**执行sql**/
//            affectRowCount = preparedStatement.executeUpdate();
//            connection.commit();
//            System.out.println("成功删除了" + affectRowCount + "行");
//        } catch (Exception e) {
//            if (connection != null) {
//                connection.rollback();
//            }
